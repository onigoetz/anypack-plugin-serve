...
Occasionally a development environment needs to mirror certain aspects of the production environment. That may mean adding custom headers to each request the server responds to. With `anypack-plugin-serve` you have two ways to do that: using [Polka](hhttps://www.npmjs.com/package/polka) underpinnings or our custom middleware. 
    ...
    new Serve({
      middleware: (app, builtins) =>
        app.use(async (req, res, next) => {
          res.setHeader('X-Superhero', 'batman');
          await next();
        })
    })
  ]
...
Notice that we're tapping into the middleware chain to do our thing. In this example, we're adding a `X-Superhero: batman` header via `res.setHeader('X-Superhero', 'batman');`. And that's about all there is to it.
