...
Occasionally a development environment needs to mirror certain aspects of the production environment. That may mean adding custom headers to each request the server responds to. With `anypack-plugin-serve` you have two ways to do that: using [Koa](https://koajs.com) underpinnings or our custom middleware. 

### Meat and Potatoes

To get started, your `webpack` configuration should already be setup and building successfully without using `anypack-plugin-serve`. Next, let's get the plugin setup for custom headers using `koa` options directly:
